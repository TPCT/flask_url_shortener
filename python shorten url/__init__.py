...
dash_board_urls = ('dashboardindex', 'dashboard')
...
class Security:
	def encryption(self, data, key=10, passphrase_len=3):
		...
		for i in range(1, len(data) + 1):
			char = ord(data[i - 1])
			passphrase = [random.choice(list(string.printable.replace('$', '').replace('\\', ''))) for x in
	...
	class Create_user(threading.Thread):
		...
		def run(self):
			...
			for user in new_users_data:
				Args = Security().decryption(eval(user).decode()).split(':')
				username = Args[0]
				password = Args[1]
...
def index():
	...
	if str(request.method).lower() == 'post':
		if form.validate_on_submit():
			...
			shorted_urls_file.write(str(
				Security().encryption(scrirpt_domain_name + 'link/%s-:-%s-:-%s' % (parsed_url, url, 0),
									  random.randint(7, 20))) + '\n')
			shorted_urls_file.close()
			database().file_watcher().shorted_urls().start()
			return render_template('index.html', form=form, valid_url=scrirpt_domain_name + 'link/%s' % parsed_url)
...
def Url_Catcher(path):
	global scrirpt_domain_name
	scrirpt_domain_name = request.host_url
	path = path.split('.')[0]
	if path.lower() in main_urls:
		return redirect('/%s' % path)
	elif path.replace('/', '') in dash_board_urls:
	...
	if sign_up_form.validate_on_submit():
		if str(request.method).lower() == 'post':
			...
			if password == confirm_password and username not in users_table.keys() and username not in new_users.keys() and email_address not in emails_address:
				link = str(scrirpt_domain_name) + 'reg/' + Security().Add_New_User(username, password,
																					email_address, phone)
				message = """
  ...
  <tbody><tr>
    <td style="padding:5px 5px" align="center" valign="middle">
              ...
              Confirm Account
				...
				message = Message('Activation Url', recipients=[email_address], html=message, sender=mail_settings['MAIL_USERNAME'])
				mail.send(message)
